; palettes for overworlds, title screen, monsters
; make some changes to red for black version :3
SuperPalettes: ; 72660 (1c:6660)
	;RGB 31,29,31 ; PAL_ROUTE
	RGB 30,30,30
	RGB 21,28,11
	RGB 20,26,31
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_PALLET
	RGB 30,30,30
	RGB 25,28,27
	RGB 20,26,31
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_VIRIDIAN
	RGB 30,30,30
	RGB 17,26,3
	RGB 20,26,31
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_PEWTER
	RGB 30,30,30
	RGB 23,25,16
	RGB 20,26,31
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_CERULEAN
	RGB 30,30,30
	RGB 17,20,30
	RGB 20,26,31
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_LAVENDER
	RGB 30,30,30
	RGB 27,20,27
	RGB 20,26,31
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_VERMILION
	RGB 30,30,30
	RGB 30,18,0
	RGB 20,26,31
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_CELADON
	RGB 30,30,30
	RGB 16,30,22
	RGB 20,26,31
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_FUCHSIA
	RGB 30,30,30
	RGB 31,15,22
	RGB 20,26,31
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_CINNABAR
	RGB 30,30,30
	RGB 26,10,6
	RGB 20,26,31
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_INDIGO
	RGB 30,30,30
	RGB 22,14,24
	RGB 20,26,31
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_SAFFRON
	RGB 30,30,30
	RGB 27,27,3
	RGB 20,26,31
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_TOWNMAP
	RGB 30,30,30
	RGB 20,26,31
	RGB 17,23,10
	RGB 3,2,2
IF DEF(_RED)
	;RGB 31,29,31 ; PAL_LOGO1
	RGB 30,30,30
	;RGB 30,30,17
	;RGB 17,23,10
	;RGB 21,0,4 ; version color
	RGB 25,25,25
	RGB 16,16,16
	RGB 0,0,0
ENDC
IF DEF(_BLUE)
	RGB 31,29,31 ; PAL_LOGO1
	RGB 30,30,17
	RGB 21,0,4
	RGB 14,19,29
ENDC
	;RGB 31,29,31 ; PAL_LOGO2
	RGB 30,30,30
	RGB 30,30,17
	RGB 18,18,24
	RGB 7,7,16
	;RGB 31,29,31 ; PAL_0F
	RGB 30,30,30
	RGB 24,20,30
	RGB 11,20,30
	;RGB 25,25,25
	;RGB 20,20,20
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_MEWMON
	RGB 30,30,30
	RGB 30,22,17
	RGB 16,14,19
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_BLUEMON
	RGB 30,30,30
	RGB 18,20,27
	RGB 11,15,23
	RGB 3,2,2
	;RGB 31,29,30 ; PAL_REDMON
	RGB 30,30,30
	RGB 31,20,10
	RGB 26,10,6
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_CYANMON
	RGB 30,30,30
	RGB 21,25,29
	RGB 14,19,25
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_PURPLEMON
	RGB 30,30,30
	RGB 27,22,24
	RGB 21,15,23
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_BROWNMON
	RGB 30,30,30
	RGB 28,20,15
	RGB 21,14,9
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_GREENMON
	RGB 30,30,30
	RGB 20,26,16
	RGB 9,20,11
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_PINKMON
	RGB 30,30,30
	RGB 30,22,24
	RGB 28,15,21
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_YELLOWMON
	RGB 30,30,30
	RGB 31,28,14
	RGB 26,20,0
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_GREYMON
	RGB 30,30,30
	RGB 26,21,22
	RGB 15,15,18
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_SLOTS1, slots are used in attacks and pokeballs
	;RGB 26,21,22
	;RGB 27,20,6
	RGB 30,30,30
	RGB 23,23,23
	RGB 17,17,17
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_SLOTS2
	;RGB 31,31,17
	RGB 30,30,30
	RGB 26,26,26
IF DEF(_RED)
	;RGB 25,17,21
	RGB 21,21,21
ENDC
IF DEF(_BLUE)
	RGB 16,19,29
ENDC
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_SLOTS3
	;RGB 22,31,16
	RGB 30,30,30
	RGB 23,23,23
IF DEF(_RED)
	;RGB 25,17,21
	RGB 21,21,21
ENDC
IF DEF(_BLUE)
	RGB 16,19,29
ENDC
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_SLOTS4
	RGB 30,30,30
IF DEF(_RED)
	;RGB 16,19,29
	;RGB 25,17,21
	RGB 23,23,23
	RGB 20,20,20
ENDC
IF DEF(_BLUE)
	RGB 25,17,21
	RGB 16,19,29
ENDC
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_BLACK
	RGB 30,30,30
	RGB 7,7,7
	RGB 2,3,3
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_GREENBAR
	RGB 30,30,30
	RGB 30,26,15
	RGB 9,20,11
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_YELLOWBAR
	RGB 30,30,30
	RGB 30,26,15
	RGB 26,20,0
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_REDBAR
	RGB 30,30,30
	RGB 30,26,15
	RGB 26,10,6
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_BADGE
	RGB 30,30,30
	RGB 30,22,17
	RGB 11,15,23
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_CAVE
	RGB 30,30,30
	RGB 21,14,9
	RGB 18,24,22
	RGB 3,2,2
	;RGB 31,29,31 ; PAL_GAMEFREAK
	RGB 30,30,30
	RGB 31,28,14
	RGB 24,20,10
	RGB 3,2,2
